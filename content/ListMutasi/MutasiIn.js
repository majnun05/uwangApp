...
const Item = ({title, trxdate, moneyRest, moneyIn}) => (
  <View style={styles.item}>
    ...
    <View>
        <Text style={[s.fs13, s.fontMSB, {color:'#0BCA6E'}]}>Rp {moneyIn}</Text>
    </View>
  </View>
);

const MutasiIn = ({debit, onEndReached, page, totalPage, load}) => {
  let loading = load
  const renderItem = ({item}) => <Item title={item.note} trxdate={item.transactionDate} moneyIn={item.balance} moneyRest={item.balanceTotal}/>;
