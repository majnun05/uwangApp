...
const ModalFilterGrosir = (props) => {
  let dtStatus = props.dataStatus ? props.dataStatus : [];

  const editStatus = (key) => {
    let edit = [];
    for (let i = 0; i < dtStatus.length; i++) {
      let fil = {
        code: dtStatus[i].code,
        name: dtStatus[i].name,
        active: key === i ? true : false,
      };
      edit.push(fil);
    }
    props.onPressStatus(edit);
  };
        ...
        <ScrollView>
          <View
            ...
            <View style={styles.PulsaLabelPhone}>
              <Text style={styles.PulsaTextPhone}>Status Transaksi</Text>
            </View>
            <View style={[styles.row, styles.mt10]}>
              <ScrollView
                showsHorizontalScrollIndicator={false}
                horizontal={true}>
                {dtStatus.map((item, key) => (
                  <View style={[styles.statusRow, styles.pr5]} key={key}>
                    <TouchableOpacity
                      onPress={() => {
                        editStatus(key);
                      }}
                      ...
                      <Text
                        style={[
                          item.active ? styles.white : styles.black,
                          styles.fontWSR,
                          styles.fs12,
                        ]}>
                        {item.name}
            ...
            <View style={[styles.PulsaLabelPhone, styles.mt10]}>
              <Text style={styles.PulsaTextPhone}>Nomor Invoice</Text>
            </View>
            <View style={styles.sectionForm}>
              <TextInput
                style={[styles.PulsaInputBoxNew, styles.black, styles.fontWSR]}
                placeholder="Cari dengan nomor invoice"
...
export default ModalFilterGrosir;
