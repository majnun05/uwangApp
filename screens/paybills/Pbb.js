...
const Pbb = (props) => {
  ...
  const [produk, setProduk] = useState('PBB');
      ...
      .then((res) => {
        if (isMounted) {
          ...
          if (res.statusCode === 200) {
            let val = res.values;
            let op = val.values.productPbb;
            setOperatorId(op);

            let kets = val.values.ketPbb;
        ...
        <View style={[styles.widgedPulsa]}>
          <View style={[styles.ml10, styles.mr10]}>
            <View style={styles.PulsaLabelPhone}>
              <Text style={styles.PulsaTextPhone}>Jenis Pajak</Text>
            </View>
            <Ripple
              onPress={() => {
                navigation.push('PbbProduk', {
              ...
              <View style={[styles.col90]}>
                <Text
                  ...
                  {productName ? productName : 'Pilih Jenis Pajak'}
                </Text>
              </View>
          ...
          <View style={{marginTop: form ? 10 : -5, marginLeft: -5}}>
            <InputPhone
              ...
              title="Masukkan Nomor Objek Pajak"
              contact={true}
              copas={true}
              minus={false}
              image={''}
              form={form2}
              titleForm="Masukkan Nomor Objek Pajak"
              maxLength={18}
            />
          </View>
