...
const TvKabelTagihan = () => {
      ...
      .then((res) => {
        if (isMounted) {
          if (res.statusCode === 200) {
            let val = res.values;
            let op = val.values.productPpob_tv;
            let kets = val.values.ketTvKabelTagihan;
  ...
  const renderItem = React.useCallback(({item, index}) => {
    return (
      <ContentOperator
        key={index}
        onPress={() =>
          navigation.push('TvKabelTagihanDetail', {
            detail: item,
  ...
  const onChangeWidget = (val) => {
    if (val === 'game') {
      ...
      navigation.navigate('TvKabelVoucher');
    } else {
      setActiveGame(false);
      setActiveOther(true);
    }
      ...
      <View style={[styles.flex1, {marginTop: -40}]}>
        ...
        <View
          ...
          <View
            ...
            <TouchableOpacity
              onPress={() => navigation.navigate('TvKabelVoucher')}
              style={{
                width: 20,
                height: 20,
                borderRadius: 20 / 2,
