...
import {apiHistoryTransfer} from '../../helpers/endPoint';
...
const RiwayatTransfer = (props) => {
  ...
  const isLogged = async (hal) => {
    ...
    await useApiPost(apiHistoryTransfer(), {
      page: hal,
      ...
      <Ripple
        ...
        <View style={[styles.col60]}>
          ...
          <Text
            ...
            numberOfLines={2}
            ellipsizeMode="tail">
            Transfer Saldo ke {item.downlineId} - {item.downlineName}
          </Text>
          <Text style={[styles.fs12, styles.green, styles.fontWSB]}>
            <Text style={[styles.fs10, styles.green, styles.fontWSR]}>
              {' '}
              Rp {getRupiah(item.nominal)}
            </Text>
          </Text>
        </View>
        {item.status ? (
          <View style={[styles.col20, styles.center]}>
           <Text style={item.status === '1' ? [styles.statusGreen] : item.status === '0' ? [styles.statusOrange] : [styles.statusRed]}>{item.status === '1' ? "Sukses" : item.status === '0' ? 'Pending' : 'Gagal'}</Text>
         </View>
        ) : null}
      </Ripple>
    );
