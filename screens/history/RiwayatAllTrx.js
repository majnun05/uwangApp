...
import FilterAllHistory from '../../content/modal/filterAllHistory';
...
import {apiHistoryAllTransaction, apiHistoryTransaction} from '../../helpers/endPoint';
...
const RiwayatAllTrx = () => {
  ...
  const isLogged = async (hal) => {
    ...
    await useApiPost(apiHistoryAllTransaction(), {
      ...
      .then((res) => {
        setIsLoading(false)
        // console.log('total data trx', res)
        if (isMounted) {
          setIsLoading(false);
  ...
  const onNavigateDetail = (item) => {
    if(item.type === 1 || item.type === 2){
      navigation.push('RiwayatDetailAllTrx', {
      ...
      <Ripple
        ...
        <View style={[styles.col20, styles.center]}>
          <Image
            resizeMethod="resize"
            source={{
              uri: item.iconUrl ? item.iconUrl : imageDefault,
            }}
