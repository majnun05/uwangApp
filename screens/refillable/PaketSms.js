...
const PaketSms = () => {
  ...
  const [ktrTelkomsel, setKtrTelkomsel] = useState({
    title: '',
    message: '',
    active: false,
  });
      ...
      .then((res) => {
        if (isMounted) {
          ...
          if (res.statusCode === 200) {
            let val = res.values;
            let kets = val.values.ketPaketSms;
            let ketsTelkomsel = val.values.ketPaketSmsTelkomsel;
            setKtr(kets ? kets : ktr);
            setKtrTelkomsel(ketsTelkomsel ? ketsTelkomsel : ktr);
          } else {
    ...
    await useApiPost(apiProductPulsa(), {
      ...
      .then((res) => {
        if (res.statusCode === 200) {
          ...
          setProvider(val.data[0].provider);
  ...
  const renderItem = React.useCallback(({item, index}) => {
    return (
      <ContentOperator
        key={index}
        onPress={() =>
          navigation.push('PaketSmsDetail', {
                ...
                {provider === 'telkomsel' ? (
                  <>
                    {ktrTelkomsel.active ? (
                      <Keterangan
                        menu="normal"
                        title={ktrTelkomsel.title}
                        msg={ktrTelkomsel.message}
                      />
                    ) : null}
                  </>
                ) : (
